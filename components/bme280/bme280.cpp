...
MZDK::BME280::BME280(ComProtocol *com_protocol) {
    ...
    com_protocol->writeRegister(CONFIG, 0); // Enable SPI 4-wire
    getCalibrateData();
    com_protocol->writeRegister(CTRL_HUM, m_humidity_oversampling);
    com_protocol->writeRegister(CTRL_MEAS, m_pressure_oversampling | m_temperature_oversampling | m_sensor_mode);
}
...
void MZDK::BME280::Init() {
    m_com_protocol->writeRegister(CONFIG, 0); // Enable SPI 4-wire
    getCalibrateData();
    m_com_protocol->writeRegister(CTRL_HUM, m_humidity_oversampling);
    m_com_protocol->writeRegister(CTRL_MEAS, m_pressure_oversampling | m_temperature_oversampling | m_sensor_mode);
}
    ...
    int MZDK::BME280::getSensorData(m_SensorRawData *resultRaw) {
        ...
        uint8_t pressMsb = m_com_protocol->readRegister(PRESS_MSB);
        uint8_t pressLsb = m_com_protocol->readRegister(PRESS_LSB);
        uint8_t pressXlsb = m_com_protocol->readRegister(PRESS_XLSB);
        uint8_t tempMsb = m_com_protocol->readRegister(TEMP_MSB);
        uint8_t tempLsb = m_com_protocol->readRegister(TEMP_LSB);
        uint8_t tempXlsb = m_com_protocol->readRegister(TEMP_XLSB);
        uint8_t humMsb = m_com_protocol->readRegister(HUM_MSB);
        uint8_t humLsb = m_com_protocol->readRegister(HUM_LSB);
    ...
    int MZDK::BME280::SetCtrlMeas(const uint8_t ctrlMeas) {
        ...
        return m_com_protocol->writeRegister(CTRL_MEAS, ctrlMeas);
    }
    ...
    int MZDK::BME280::SetTemperatureOversampling(const uint8_t tempOversampling) { // ctrl_meas bits 7, 6, 5   page 29
        ...
        return m_com_protocol->writeRegister(CTRL_MEAS, temp | tempOversampling);
    }
    ...
    int MZDK::BME280::SetPressureOversampling(const uint8_t pressureOversampling) { // ctrl_meas bits 4, 3, 2
        ...
        return m_com_protocol->writeRegister(CTRL_MEAS, temp | pressureOversampling);
    }
    ...
    int MZDK::BME280::SetOversampling(const uint8_t tempOversampling, const uint8_t pressureOversampling) {
        ...
        return m_com_protocol->writeRegister(CTRL_MEAS, tempOversampling | pressureOversampling | m_sensor_mode);
    }
    ...
    int MZDK::BME280::SetCtrlHum(const int humididtyOversampling) { // ctrl_hum bits 2, 1, 0    page 28
        ...
        return m_com_protocol->writeRegister(CTRL_HUM, humididtyOversampling);
    }
    ...
    int MZDK::BME280::Reset(void) { // writeRegister 0xB6 into reset (0xE0)
